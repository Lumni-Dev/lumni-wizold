export const AI_AUDIT_NOTICE =
  "Nomes passam por auditoria de IA antes de entrar no jogo. Mensagens da taverna são auditadas depois do envio, com regra mais flexível: só o que for claramente impróprio é removido.";

export const MODERATION_REFUSAL =
  "O texto foi recusado pela auditoria de IA. Escolha outras palavras.";

export const MODERATION_UNAVAILABLE =
  "A auditoria de IA está indisponível. Tente de novo em instantes.";

export const CHAT_CENSORED_TEXT = "Conteúdo impróprio removido.";
