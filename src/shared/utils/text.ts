...
// The game's administrator wears the [ADM] mark wherever the nick shows. The
// stored name stays clean: uniqueness, search and mentions read the real nick,
// only the display decorates it.
export const ADMIN_NAME = "Lumni";

export function displayNick(name: string): string {
  return name === ADMIN_NAME ? "[ADM] " + name : name;
}
