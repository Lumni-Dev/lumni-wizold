...
// The wolf's recovery on the server clock: each real minute at home pays one
// restPetTick, and the stamp advances exactly the minutes paid.
export async function POST(request: Request) {
  return withGame(request, async (state, _body, context) => {
    ...
    return success(current, whole && current.pet ? current.pet.name + " está de pé, inteiro e pronto." : "", {
      whole,
      ticks,
    });
  });
