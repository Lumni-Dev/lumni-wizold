// End-to-end smoke: a throwaway account plays against the real database
// through the API, the rows are checked underneath, and everything the run
// touched is deleted by cascade at the end, so it is safe to repeat.
//   npm run smoke   (needs next dev on :3000 and .env.local pointing at PG)
...
// 4. A bad name refuses; a good one creates.
...
// 5. The state comes from the database with the right beginnings.
...
// 6. Turns beast (rage 100, cost 40) and hunts.
...
// 7. The market charges and delivers.
const bought = await call("POST", "/api/market/buy", { itemId: "health-potion-small", quantity: 1 });
...
// 8. Training, beast only, charging bronze.
...
const spoke = await call("POST", "/api/tavern/rooms/" + roomId + "/messages", { text: "Uivo de teste" });
...
check("mesa listada com a fala", seat?.room?.messages?.some((m) => m.text === "Uivo de teste") === true);

// 11. The bazaar answers with the roster board.
...
// ---- now the database, underneath the API -----------------------------------
...
const client = new pg.Client({ database: "postgres", ssl: { rejectUnauthorized: false } });
...
const movement = await rows(
  "select reason from wallet_movements where character_id = $1 order by id", [character?.id]);
check("ledger tem o saldo inicial", movement[0]?.reason === "starting_balance");
const diary = await rows("select count(*)::int as n from log_entries where character_id = $1", [character?.id]);
...
// ---- cleanup: the throwaway account leaves by cascade ------------------------
